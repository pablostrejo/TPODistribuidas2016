...
//	}
	...
	private Date obtenerFechaTrayectoPedido(Pedido p, RutaDTO ruta) {
		...
		for(TrayectoDTO t : ruta.getTrayectos()){
			...
			if(t.getSucursalDestino().equals(p.getSucursalDestino())){
				long m =(long) tiempo;
	...
	private RutaDTO obtenerRuta(Pedido p, MapaDeRutaDTO mp) {
		...
		String sucOrigen  = p.getSucursalOrigen();
		String sucDestino = p.getSucursalDestino(); 
		
		
		for(RutaDTO r: mp.getRutas()){
			boolean sucOrigenAux = false;
			for(TrayectoDTO t : r.getTrayectos()){
				if(t.getSucursalOrigen().equals(sucOrigen)){
					sucOrigenAux = true;
				}
				if(t.getSucursalDestino().equals(sucDestino) && (sucOrigenAux)){
					return r;
				}
			}
		}

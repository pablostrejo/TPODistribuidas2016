...
//	}
	...
	private void llenarVehiculosParaEnvio(int sucursal, List<ViajeDTO> viajes, List<Vehiculo> vehiculosDisp, MapaDeRutaDTO mp) {
		...
		if (vehiculosDisp != null) {
			for (Vehiculo v : vehiculosDisp) {
				...
				RutaDTO r = llenarVehiculo(sucursal, mp, v.getVolumen(), ped);
	...
	private RutaDTO llenarVehiculo(int sucursal, MapaDeRutaDTO mp, int volumenVehiculo, List<Pedido> pedidosReturn) {
		List<Pedido> pedidos = hbtDAO.obtenerPedidosEnDeposito(sucursal);
		...
		float vol70 = (volumenVehiculo*70/100);
		...
		if (pedidos != null) {
			for (Pedido p : pedidos) {
				...
				if (cont) {
					...
					if (volumneCargasAcum <=volumenVehiculo) {
						pedidosReturn.add(p);
					} else {
		...
		if (volumneCargasAcum >= vol70 && volumneCargasAcum <=volumenVehiculo) {
			return r;
		} else {
			return null;
		}
	...
	private boolean vaHaciaLaRuta (Pedido p, RutaDTO r) {
		
		boolean sucOrigenAux = false;
		for(TrayectoDTO t : r.getTrayectos()){
			if(t.getSucursalOrigen().getIdSucursal() == Integer.parseInt(p.getSucursalOrigen())){
				sucOrigenAux = true;
			}
			if((t.getSucursalDestino().getIdSucursal() == Integer.parseInt(p.getSucursalDestino())) && (sucOrigenAux)){
				return true;
			}
		}
	...
	private void generarViajeMultiplePedidos(List<ViajeDTO> viajes, List<Pedido> pedidos, RutaDTO ruta, int idVehiculo) {
